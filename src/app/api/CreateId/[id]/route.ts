...
export async function POST(request: Request) {
  try {
    ...
    const baseUrl = request.headers.get('x-forwarded-host') || new URL(request.url).host; // Use x-forwarded-host if available
    const thankYouUrl = `http://${baseUrl}/thank-you`; // Construct the full URL for the ThankYou page
